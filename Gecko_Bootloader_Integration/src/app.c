...
void app_init(void)
{
  ...
  if((btl_status = bootloader_init()) != BOOTLOADER_OK) { // init bootloader and check if it was successful
      otCliOutputFormat("BTL: init error: 0x%08lx\n", btl_status);
  } else {
      otCliOutputFormat("BTL: init OK\n");
      bootloaderInterfaceCliInit();
  }

  otCliOutputFormat("Gecko Bootloader Integration Started App v %d\r\n", applicationProperties.app.version);
}
